...
xhr.onload = function() {                       // When readystate changes
  ...
  //if(xhr.status === 200) {                      // If server status was ok
    ...
    var newContent = '';
    for (var i = 0; i < responseObject.ourcode.length; i++) { // Loop through object
      newContent += '<div class="our_code col-md-4">';
      newContent += '<img src="' + responseObject.ourcode[i].icon + '" ';
      newContent += 'alt="' + responseObject.ourcode[i].desc + '" />';
      newContent += '<p><b>' + responseObject.ourcode[i].desc + '</b><br>';
      newContent += responseObject.ourcode[i].codedata + '</p>';
      newContent += '</div>';
    }
...
xhr.open('GET', 'api/data.json', true);        // Prepare the request
